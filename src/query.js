            ...
            var valueVariations = {};
                ...
                for(var i = 0; i < orderBy.length; i++){
                    ...
                    for(var j = 0; j < currentOrderBy.length - 1; j++){
                        accumulationObjectBuffer = accumulationObjectBuffer[orderBy[i][j]];
                    }
...
jsonOdm.Query.prototype.$accumulator = function (nodes,accumulator) {
    var subQuery = new jsonOdm.Query(this.$$collection);
    var $accumulator = (function(nodes,accumulator,query){
        /**
         * @param {*} The collection to go down
         * @return {Query|boolean} The query object with the sub collection or false if querying was impossible
         */
        return function(collection){
            var value = jsonOdm.util.branch(collection,nodes);
            query.$$accumulation = accumulator(value,query.$$accumulation);
            return value;
        };
    })(nodes,accumulator,subQuery);
...
jsonOdm.Query.prototype.$sum = function (branch) {
    return this.$accumulator(typeof branch == "string"?[branch]:branch, function (value,accumulation) {
